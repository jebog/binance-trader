...
Personal macOS algorithmic scanner that monitors 6 spot pairs every 30 minutes, detects multi-tier RSI + SMA + volume + sentiment buy signals, and places confirmed market orders with automatic ATR-based OCO exit brackets.
...
2. **Max positions** — skip a signal if `MAX_POSITIONS` are already open
